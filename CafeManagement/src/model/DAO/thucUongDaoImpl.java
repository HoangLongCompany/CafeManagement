...
public class thucUongDaoImpl implements thucUongDao{
    ...
    public void delete(String tuID) {
        conn = DBConnection.getConnection();
        String sql = "DELETE FROM tb_thucuong WHERE ThucUongID = ? ";    
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, tuID);
            pst.executeUpdate();
        } catch (SQLException e) {
            Logger.getLogger(thucUongDaoImpl.class.getName()).log(Level.SEVERE, null, e);
        }
    ...
    public thucUong getThucUong(String tuID) {
        conn = DBConnection.getConnection();
        thucUong tu = null;
        String sql = "SELECT * FROM tb_thucuong WHERE ThucUongID = ? ";
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, tuID);
            rs = pst.executeQuery();
...
//        thucUongDao.add(tu);
//        thucUong tu = thucUongDao.getThucUong("TU001");
//        System.out.println(tu.toString());
//        List<thucUong> l = new ArrayList<>();
//        l = thucUongDao.getAll();
//        System.out.println(l.size());
//        for (thucUong i : l){
//            System.out.println(i.toString());
//        }
//        thucUongDao.delete("TU001");
